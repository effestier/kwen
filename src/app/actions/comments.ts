...
export async function getComments(postId: string, limit = 50): Promise<Comment[]> {
  try {
    ...
    // Fetch parent comments only (parent_id IS NULL)
    ...
    // Fetch reply counts for all parent comments in one query
    ...
    // Fetch like counts for all parent comments
...
export async function addComment(
  ...
  try {
    ...
    // If replying, set parent_id (always points to top-level parent)
    if (parentId) {
