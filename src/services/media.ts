...
export async function uploadAvatar(filePath: string) {
  try {
    ...
    if (!filePath || !isValidMediaUrl(filePath)) return { error: 'Invalid file path' };
...
export async function uploadPostMedia(postId: string, urls: string[]) {
  try {
    ...
    if (!Array.isArray(urls) || urls.length === 0 || urls.length > 10) return { error: 'Invalid media count' };

    for (const url of urls) {
      if (!isValidMediaUrl(url)) return { error: 'Invalid media URL' };
    }
    ...
    if (error) return { error: 'Failed to save media' };
    return { success: true };
  } catch {
...
export async function deletePostMedia(mediaId: string) {
  try {
    ...
    if (!user) return { error: 'Not authenticated' };
    if (!mediaId || typeof mediaId !== 'string') return { error: 'Invalid media ID' };
...
export async function createPostWithMedia(formData: FormData) {
  try {
    ...
    if (!content && !mediaUrlsRaw) return { error: 'Post must have content or images' };
    ...
    if (mediaUrlsRaw) {
      try {
        ...
        if (!Array.isArray(urls) || urls.length > 10) return { error: 'Too many media files' };
        for (const url of urls) {
          if (!isValidMediaUrl(url)) return { error: 'Invalid media URL' };
        }
...
export async function uploadStory(mediaUrl: string, mediaType: string = 'image') {
  try {
    ...
    if (!user) return { error: 'Not authenticated' };
    if (!isValidMediaUrl(mediaUrl)) return { error: 'Invalid media URL' };
